...
<style>
  ...
  *{ margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ---------- THE QUESTION ---------- */
  .question-header{ text-align:center; margin-bottom:56px; }
  .question-header h2{ font-size:clamp(1.8rem, 4vw, 2.6rem); }
  .question-header p{ color:var(--text-soft); margin-top:12px; }
  ...
  .q-yes:hover{ background:var(--gold); color:var(--bg); }
  ...
  .q-maybe.dodging{
    position:absolute;
    transition: left 0.35s cubic-bezier(.2,.8,.3,1.2), top 0.35s cubic-bezier(.2,.8,.3,1.2);
  }
...
<section id="the-question">
  <div class="question-header reveal">
    <span class="eyebrow">one honest question</span>
    <h2>Okay, be honest.</h2>
    <p>It's a simple question. Just pick one.</p>
  </div>

  <div class="question-box reveal">
    <p class="question-text">Do you like me?</p>
    <div class="btn-stage" id="btnStage">
      <button class="q-btn q-yes" id="btnYes" type="button">yes, I like you</button>
      <button class="q-btn q-maybe" id="btnMaybe" type="button">maybe not</button>
    </div>
    <p class="dodge-hint" id="dodgeHint">&nbsp;</p>
  </div>

  <div class="question-result" id="questionResult">
    <p>Yes. Yeah. I knew it. I knew you liked me too.</p>
  </div>
</section>
...
<script>
  ...
  // The question: yes / maybe-not (which dodges the cursor/finger)
  ...
  function dodgeMaybeButton(){
    if(reduceMotion) return; // don't play the chase game if user prefers reduced motion
    const stageRect = btnStage.getBoundingClientRect();
    const btnRect = btnMaybe.getBoundingClientRect();

    if(!btnMaybe.classList.contains('dodging')){
      // First dodge: lock in current size, switch to absolute positioning within the stage
      btnMaybe.style.width = btnRect.width + 'px';
      btnMaybe.classList.add('dodging');
    }
    ...
    btnMaybe.style.left = newX + 'px';
    btnMaybe.style.top = newY + 'px';

    dodgeCount++;
    dodgeHint.textContent = dodgeHints[Math.min(dodgeCount - 1, dodgeHints.length - 1)];
  }

  btnMaybe.addEventListener('mouseenter', dodgeMaybeButton);
  btnMaybe.addEventListener('touchstart', (e) => {
    e.preventDefault();
    dodgeMaybeButton();
  }, { passive:false });
  btnMaybe.addEventListener('click', (e) => {
    // Safety net in case a click ever lands on it
    e.preventDefault();
    dodgeMaybeButton();
  });
  ...
  btnYes.addEventListener('click', () => {
    ...
    questionResult.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'center' });
  });
</script>
...
# so all sections (hero, timeline, qualities, notes, the question, message) are reachable.
